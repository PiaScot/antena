<script lang="ts">
	...
	async function handleDelete() {
		...
		try {
			const res = await fetch(`/api/category?id=${cat.id}`, {
				method: 'DELETE'
			});
...
{#if $isLayoutEditMode}
	<div
		class="relative flex h-16 items-center rounded-xl bg-white p-2 text-slate-800 shadow-md md:h-20 dark:bg-slate-600 dark:text-slate-100"
	...
	>
		<span
			class="absolute top-1/2 left-2 -translate-y-1/2 text-slate-400 dark:text-slate-300"
		>
		...
		<div class="flex-1 px-8 text-center">
			{#if isEditing}
				<input
					...
					class="w-full rounded bg-slate-100 text-center text-lg font-semibold focus:ring-2 focus:ring-emerald-500 focus:outline-none dark:bg-slate-700"
					onblur={handleUpdate}
		...
		<button
			class="absolute top-1/2 right-2 -translate-y-1/2 rounded-full p-1 text-slate-400 hover:text-red-500 dark:text-slate-300"
			title="このカテゴリを削除"
...
{:else}
	<a
		...
		class="flex h-16 items-center justify-center rounded-xl bg-slate-100 p-2 text-center text-xl font-semibold text-emerald-700 shadow transition-colors hover:bg-emerald-50 md:h-20 dark:bg-slate-700 dark:text-emerald-300 dark:hover:bg-emerald-800"
	>
		{cat.label}
	</a>
{/if}
