<script lang="ts">
	...
	import { Plus, Search, LoaderCircle, ChevronUp } from '@lucide/svelte';
	...
	<div class="mt-4 flex flex-col items-center gap-4 sm:flex-row">
		<div class="relative w-full flex-grow">
			<input
				...
				class="w-full rounded-md border border-slate-300 bg-white px-3 py-2 pl-10 text-slate-800 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 focus:outline-none dark:border-slate-600 dark:bg-slate-800 dark:text-white"
			/>
		...
		<button
			...
			class="flex w-full flex-shrink-0 items-center justify-center gap-2 rounded-md bg-emerald-600 px-4 py-2 font-semibold text-white transition hover:bg-emerald-700 sm:w-auto"
		>
			{#if showAddSiteForm}
...
{#if showAddSiteForm}
	<div class="p-4">
		<div
			class="mx-auto max-w-xl rounded-xl bg-slate-100 p-4 shadow-md dark:bg-slate-800"
		>
			...
			{#if addSiteState.successMsg}
				<div
					class="mb-2 rounded-lg bg-emerald-500/20 p-3 text-center text-emerald-500"
				>
					{addSiteState.successMsg}
				</div>
			{/if}
			{#if !addSiteState.previewSite}
				<div class="flex flex-col gap-3">
					...
					>
						...
						{/if}
						<span
							>{#if addSiteState.isProcessing}確認中...{:else}サイト情報を確認{/if}</span
						>
					</button>
				</div>
			{:else}
				...
				<div class="space-y-4">
					<div class="rounded-lg bg-slate-200 p-3 text-sm dark:bg-slate-700">
						<div>
							<span class="font-semibold">サイト名:</span>
							{addSiteState.previewSite.title}
						</div>
						<div>
							<span class="font-semibold">URL:</span>
							{addSiteState.previewSite.url}
						</div>
						<div>
							<span class="font-semibold">RSS:</span>
							{addSiteState.previewSite.rss}
						</div>
						<div>
							<span class="font-semibold">カテゴリ:</span>
							{categories.find((c) => c.id === addSiteState.inputCategory)
								?.label}
						</div>
					</div>
					{#if addSiteState.previewArticles && addSiteState.previewArticles.length > 0}
						<div>
							<h4
								class="mb-2 text-base font-semibold text-slate-800 dark:text-white"
							>
								最新記事プレビュー (3件)
							</h4>
							<ul class="space-y-2">
								{#each addSiteState.previewArticles.slice(0, 3) as article (article.url)}
									<li
										class="rounded-lg bg-slate-200 p-2 text-sm dark:bg-slate-700"
									>
										<a
											href={article.url}
											target="_blank"
...
{#if categoryModalState.show}
	...
	>
		...
		>
			<h3 class="mb-4 text-lg font-bold text-slate-800 dark:text-white">
				新規カテゴリ追加
			</h3>
