...
<section class="p-4">
	<h1 class="mb-6 text-2xl font-bold text-slate-800 dark:text-slate-100">設定</h1>
	<div class="space-y-4">
		<div
		...
		>
			<span class="text-base text-gray-900 dark:text-gray-100">ダークモード</span>
			<label class="relative inline-flex cursor-pointer items-center">
				...
				/>
				<div
					class="h-6 w-11 rounded-full bg-gray-300 after:absolute after:left-[2px] after:top-[2px] after:h-5 after:w-5 after:rounded-full after:border after:border-gray-300 after:bg-white after:content-[''] after:transition-all peer-focus:outline-none peer-focus:ring-2 peer-focus:ring-emerald-500 peer-checked:after:translate-x-full dark:bg-gray-600 peer-checked:bg-emerald-500"
				></div>
			</label>
		</div>
		...
		>
			...
			>
				{#each sizes as size (size.value)}
					<button
						type="button"
						class="px-4 py-1.5 text-sm font-semibold transition-colors duration-200 rounded-md"
						...
						class:dark:hover:bg-slate-600={$userSettings.fontSize !== size.value}
