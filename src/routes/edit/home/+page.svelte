...
$effect(() => {
	displayGroups = JSON.parse(JSON.stringify($superCategoryGroups || []));
});

function handleSuperCategoryDrop(
	e: CustomEvent<{ items: SuperCategoryGroupType[]; info: any }>,
) {
	displayGroups = e.detail.items;
	// TODO
	// ここでAPIを呼び出して、サーバー上の順序も更新するロジックを追加することも可能
}
